	...
	for (u32 i = 0; i < 256;) {
		...
		if (color_256) {
			for (; px < 8 && i < 256; px++, i++, char_row >>= 8) {
				...
				draw_bg_pixel(gpu, bg, i, color, bg_priority,
...
static void
draw_bg_pixel(gpu_2d_engine *gpu, int bg, u32 fb_x, u16 color, u8 priority,
