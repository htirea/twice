...
arm_on_cpsr_write(arm_cpu *cpu)
{
	...
	case 0x1F:
		new_mode = arm_cpu::MODE_SYS;
		break;
	case 0x11:
		new_mode = arm_cpu::MODE_FIQ;
		break;
	case 0x13:
		new_mode = arm_cpu::MODE_SVC;
		break;
	case 0x17:
		new_mode = arm_cpu::MODE_ABT;
		break;
	case 0x12:
		new_mode = arm_cpu::MODE_IRQ;
		break;
	case 0x1B:
		new_mode = arm_cpu::MODE_UND;
		break;
	case 0x10:
		new_mode = arm_cpu::MODE_USR;
		break;
	default:
...
arm_do_irq(arm_cpu *cpu)
{
	...
	cpu->cpsr &= ~0xBF;
	cpu->cpsr |= 0x92;
	arm_switch_mode(cpu, arm_cpu::MODE_IRQ);
	cpu->interrupt = false;
...
swap_registers(arm_cpu *cpu, u32 old_mode, u32 new_mode)
{
	...
	if (new_mode == arm_cpu::MODE_FIQ || old_mode == arm_cpu::MODE_FIQ) {
		for (int i = 0; i < 5; i++) {
