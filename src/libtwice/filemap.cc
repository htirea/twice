...
FileMap::FileMap(const std::string& pathname, std::size_t limit, int mode)
{
	...
	if (fd == -1) {
		throw TwiceException("could not open file: " + pathname);
	}

	struct stat s;
	if (fstat(fd, &s)) {
		close(fd);
		throw TwiceException("could not stat file: " + pathname);
	}
	...
	if (mode == MAP_EXACT_SIZE && actual_size != limit) {
		close(fd);
		throw TwiceException("file does not match size: " + pathname);
	} else if (mode == MAP_MAX_SIZE && actual_size > limit) {
		close(fd);
		throw TwiceException("file exceeds size: " + pathname);
	}

	auto addr = mmap(NULL, actual_size, PROT_READ, MAP_PRIVATE, fd, 0);
	if (addr == MAP_FAILED) {
		close(fd);
		throw TwiceException("mmap failed: " + pathname);
	}
